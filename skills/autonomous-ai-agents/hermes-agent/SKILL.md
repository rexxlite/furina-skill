...
For DM-to-group/forum-topic routing patterns, topic IDs, and per-topic rules, see `references/telegram-topic-workflows.md`.
...
- **Telegram group topics have separate context**: DM instructions do not automatically transfer into group/forum-topic sessions. For cross-topic workflows, collect and persist the target as `telegram:<chat_id>:<thread_id>`, then seed each topic with a short rules message or configure scheduled deliveries to that target. If the bot is in a forum supergroup, `chat_id` is the supergroup id (often `-100...`) and `thread_id` is the topic/message thread id. Mentioning/replying to the bot may be required when BotFather privacy mode is enabled.
